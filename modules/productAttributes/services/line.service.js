...
export const LineService = {
  getAll: async (token, page = 1, pageSize = 10) => {
    const response = await fetch(`${CONFIG.API_URL}/Lines?Page=${page}&PageSize=${pageSize}`, {
      method: 'GET',
  ...
  getById: async (token, id) => {
    const response = await fetch(`${CONFIG.API_URL}/Lines/${id}`, {
      method: 'GET',
      headers: buildHeaders(token, false)
    });
    return await parseJsonSafe(response);
  ...
  create: async (token, dto) => {
    const response = await fetch(`${CONFIG.API_URL}/Lines`, {
      method: 'POST',
